...
serve(async (req) => {
  ...
  try {
    ...
    const kieResponse = await fetch('https://api.kie.ai/v1/veo3/generate', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        firstFrameImage: imageSignedUrl,
        aspectRatio: aspectRatio === '9:16' ? 'vertical' : 'horizontal',
        durationSeconds: durationSeconds,
        notifyUrl: `${supabaseUrl}/functions/v1/video-callback`,
      }),
    });
    ...
    if (!kieResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ error: 'Erreur API Kie.ai', details: errorText }),
