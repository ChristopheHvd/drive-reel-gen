...
/**
 * Tests de Non-Régression pour le Routing SPA avec HashRouter
 * Vérifie que toutes les routes fonctionnent correctement au refresh
 * et lors d'un accès direct via URL avec hash routing
 */
test.describe('SPA Routing - HashRouter No 404', () => {
  ...
  test('should show NotFound component for invalid route', async ({ page }) => {
    ...
    // et non une erreur 404 du serveur (HashRouter = toujours index.html)
  ...
  test('should handle page refresh on /#/pricing', async ({ page }) => {
    // Naviguer vers pricing avec hash
    await page.goto('/#/pricing');
    await expect(page.locator('main')).toBeVisible();
  ...
  test('should handle navigation between hash routes', async ({ page }) => {
    // Démarrer sur home
    await page.goto('/#/');
    await expect(page.locator('main')).toBeVisible();
